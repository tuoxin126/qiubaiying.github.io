...
public class KafkaUtils {
	...
	private static final String ZOOKEEPER_CONNECT = "synhadoop123:2181,synhadoop124:2181,synhadoop125:2181";
	...
	private static ZkUtils getZkUtils(String zookeeperConnect, int sessionTimeout, int connectTimeout) {
		...
		try {
			zkUtils = ZkUtils.apply(ZOOKEEPER_CONNECT, SESSION_TIMEOUT, CONNECT_TIMEOUT,
	...
	public static void createTopic(String topic, int numPartitions, int numReplications, Properties properties) {
		...
		try {
			if (!AdminUtils.topicExists(zkUtils, topic)) {
				AdminUtils.createTopic(zkUtils, topic, numPartitions, numReplications, properties,
	 ...
	 */
	public static void addPartitions(String brokerList, String topic, int numAddPartitions,
		...
		ZkUtils zkUtils = getZkUtils(ZOOKEEPER_CONNECT, SESSION_TIMEOUT, CONNECT_TIMEOUT);
		try {
			AdminUtils.addPartitions(zkUtils, topic, partitionInfos.size() + numAddPartitions, replicaAssignmentStr2,
